...
if(isset($_POST["submit"])){
    $image_size=round($_FILES["uploadedimage"]["size"] / 1024 );
    $target_dir = "images/";
    $target_file = $target_dir . basename($_FILES["uploadedimage"]["name"]);
    move_uploaded_file($_FILES['uploadedimage']['tmp_name'], $target_file);

    
    $qry = mysqli_query($conn,"INSERT into files_upload values('".$user_id."','','".$_FILES["uploadedimage"]["name"]."','".$image_size."','".date("Y-m-d")."','Not approved','')") or die ("Cannot query with database table");
